...
1. (당시 구조 기준) Material UI 앱은 `posts/catalog.json`을 먼저 읽어 카테고리와 메타데이터를 가져왔지만, 현재 버전에서는 `docs/categories.cfg`와 프런트 매터가 같은 역할을 합니다.
...
> **Tip**: 새 글을 작성할 때는 `docs/categories.cfg`에 카테고리 정보를 추가하고, 동일한 폴더에 프런트 매터가 포함된 `.md` 파일을 저장한 뒤 `make static-site`만 실행하면 됩니다.
