...
- [ ] `docs/categories.cfg`와 `docs/posts/` 최신 상태 확인
- [ ] 브랜치 명명 규칙 `feature/<slug>` 또는 `fix/<topic>` 준수
- [ ] `make static-site`가 로컬에서 정상 동작하는지 확인

## 2. 문서 작성 단계

1. **카테고리 선택**: 없으면 `docs/categories.cfg`에 새 섹션을 추가하고 `docs/posts/<새카테고리>/` 폴더를 만듭니다.
...
2. 카테고리 색상이나 설명을 바꿨다면 `docs/categories.cfg` diff를 꼼꼼히 검토합니다.
3. Markdown 프런트 매터 양식을 변경했다면 README 및 다른 문서에도 반영합니다.

## 4. PR 체크리스트

- [ ] `npm` 의존성이 없으므로 `package-lock.json` 등은 커밋하지 않음
- [ ] `docs/categories.cfg`에 누락된 항목이 없는지 확인
...
- 필요하다면 Welcome 카테고리에 “무엇이 바뀌었는지” 요약 포스트를 작성해 독자에게 안내합니다.
